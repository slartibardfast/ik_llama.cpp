// test-mmq-q4-0-ar16-mma
//
// Phase A.5 binding (PHASE_MMQ_Q4_0_AR16.md §11): verify
// vec_dot_q4_0_ar16_q8_1_mma (INT8 tensor-core path) computes a Q4_0_AR16
// × Q8_1 dot product matching a scalar fp32 reference (cos ≥ 0.9999,
// NMSE ≤ 1e-4) AND produces byte-identical output to the A.4 DP4A path.
//
// Strategy:
...
__global__ void test_vec_dot_q4_0_ar16_mma_kernel(
    ...
    __syncthreads();
    vec_dot_q4_0_ar16_q8_1_mma<MMQ_X_T, MMQ_Y_T, NWARPS_T>(tile_x, tile_y, sum, /*k00=*/0);
    __syncthreads();
    ...
    __syncthreads();
    vec_dot_q4_0_ar16_q8_1_mma<MMQ_X_T, MMQ_Y_T, NWARPS_T>(tile_x, tile_y, sum, /*k00=*/WARP_SIZE);
    __syncthreads();

    // Write per-thread sum slots into dst per MMA's mma_int_C_I16J8 convention.
    // Each thread (tx, ty) holds C.x[4] for the 16x8 fragment slice:
    //   l=0: i = i0 + n*16 + tx/4,        j = j0_offset + 2*(tx%4)
    //   l=1: i = i0 + n*16 + tx/4,        j = j0_offset + 2*(tx%4) + 1
    //   l=2: i = i0 + n*16 + 8 + tx/4,    j = j0_offset + 2*(tx%4)
    //   l=3: i = i0 + n*16 + 8 + tx/4,    j = j0_offset + 2*(tx%4) + 1
    // Where i0 = (ty/ntx)*rows_per_warp.
    constexpr int granularity = mmq_get_granularity_device(MMQ_X_T);
    constexpr int rows_per_warp = 2 * granularity;
    constexpr int mma_I = 16, mma_J = 8, mma_ne = 4;
    constexpr int ntx = rows_per_warp/mma_I;
    constexpr int n_j_tiles = MMQ_X_T / mma_J;
    constexpr int sum_n_per_tile = mma_ne;
    const int i0 = (threadIdx.y/ntx)*rows_per_warp;

    #pragma unroll
    for (int j_tile = 0; j_tile < n_j_tiles; ++j_tile) {
        #pragma unroll
        for (int n = 0; n < ntx; ++n) {
            const int slot_base = (j_tile == 0 ? n : j_tile + n)*sum_n_per_tile;
            #pragma unroll
            for (int l = 0; l < mma_ne; ++l) {
                const int i = i0 + n*mma_I + (l/2)*(mma_I/2) + threadIdx.x / (mma_J/2);
                const int j = j_tile*mma_J + 2*(threadIdx.x % (mma_J/2)) + (l%2);
                if (i < MMQ_Y_T && j < MMQ_X_T) {
                    dst[i*MMQ_X_T + j] = sum[slot_base + l];
                }
            }
        }
    }
...
static bool run_config(std::mt19937 & rng) {
    ...
    dim3 grid(1, 1, 1);
    test_vec_dot_q4_0_ar16_mma_kernel<MMQ_X, MMQ_Y, NWARPS><<<grid, block, smem_bytes>>>(
...
int main() {
    ...
    // MMA kernel constraints: nwarps*rows_per_warp >= mmq_y (rows_per_warp=16
    // for mmq_x<48, so nwarps>=mmq_y/16 needed); mmq_x % 8 == 0.
    // Production uses nwarps=MMQ_NWARPS=8, mmq_y=128.
    ok &= run_config< 8,  64, 4>(rng);   // small variant
    ok &= run_config<16,  64, 4>(rng);
    ok &= run_config<32,  64, 4>(rng);
    ok &= run_config< 8, 128, 8>(rng);   // production-shape
    ok &= run_config<16, 128, 8>(rng);
    ok &= run_config<32, 128, 8>(rng);
