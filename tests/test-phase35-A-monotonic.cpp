// Phase 35 A.T3: hit-counter monotonicity.
//
...
//
// RED at A.0 (probe_flush stub returns -1; no dump emitted).
// GREEN after A.1 (hit counter on cache entries) + A.7 (dump infra).
//
...
int main() {
    char dir_template[] = "phase35-A-monotonic-XXXXXX";
    ...
    printf("=== test-phase35-A-monotonic ===\n");
    printf("  drove %d same-shape submits (n=8)\n", N_REPEATS);
    printf("  probe_flush rc = %d\n", flush_rc);

    if (flush_rc != 0) {
        printf("RESULT: RED — probe_flush did not write a dump (stub returns -1 until A.7)\n");
        return 1;
    }
