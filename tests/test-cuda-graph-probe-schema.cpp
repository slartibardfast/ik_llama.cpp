// CUDA graph cache: probe dump JSONL schema validation.
//
// Drives a small workload, flushes, and walks every JSONL record asserting
// the per-probe required fields. Common required fields: ts_ns, runid,
// backend, probe. Per-probe required fields are checked in validate().
//
// RED until at least the hit_counter records and the JSONL dump
...
int main() {
    char dir_template[] = "cuda-graph-probe-schema-XXXXXX";
    ...
    printf("=== test-cuda-graph-probe-schema ===\n");
