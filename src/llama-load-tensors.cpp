...
bool create_tensors_helper::create_llama_tensors(const LLM_TN & tn) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        layer.rope_freqs = create_tensor(ctx_split, tn(LLM_TENSOR_ROPE_FREQS, "weight"), {n_rot/2}, llama_model_loader::TENSOR_NOT_REQUIRED | (i != 0 ? llama_model_loader::TENSOR_DUPLICATED : 0));
...
bool create_tensors_helper::create_qwen35moe_tensors(const LLM_TN & tn) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (!hparams.is_recurrent(i)) {
            // Attention layers (including MTP layers which use standard attention)
        ...
        // NextN/MTP tensors for the final nextn_predict_layers
        if (hparams.nextn_predict_layers > 0 && static_cast<uint32_t>(i) >= n_layer - hparams.nextn_predict_layers) {
            layer.nextn.eh_proj          = create_tensor(ctx_split, tn(LLM_TENSOR_NEXTN_EH_PROJ, "weight", i),
                                            { 2*n_embd, n_embd }, flags);
            layer.nextn.embed_tokens     = create_tensor(ctx_split, tn(LLM_TENSOR_NEXTN_EMBED_TOKENS, "weight", i),
                                            { n_embd, n_vocab }, flags | llama_model_loader::TENSOR_NOT_REQUIRED);
            layer.nextn.enorm            = create_tensor(ctx_split, tn(LLM_TENSOR_NEXTN_ENORM, "weight", i),
                                            { n_embd }, flags);
            layer.nextn.hnorm            = create_tensor(ctx_split, tn(LLM_TENSOR_NEXTN_HNORM, "weight", i),
                                            { n_embd }, flags);
            layer.nextn.shared_head_head = create_tensor(ctx_split, tn(LLM_TENSOR_NEXTN_SHARED_HEAD_HEAD, "weight", i),
                                            { n_embd, n_vocab }, flags | llama_model_loader::TENSOR_NOT_REQUIRED);
            layer.nextn.shared_head_norm = create_tensor(ctx_split, tn(LLM_TENSOR_NEXTN_SHARED_HEAD_NORM, "weight", i),
                                            { n_embd }, flags | llama_model_loader::TENSOR_NOT_REQUIRED);
        }
    }
