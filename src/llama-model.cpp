...
static const std::map<llm_arch, std::map<llm_tensor, std::string>> LLM_TENSOR_NAMES = {
    ...
    },
    {
        ...
        {
            // Trunk surfaces. No token_embd, no output (lm_head):
            // the drafter delegates both to the paired target.
            { LLM_TENSOR_OUTPUT_NORM,        "output_norm" },
            { LLM_TENSOR_DFLASH_FC,          "dflash.fc" },
            { LLM_TENSOR_DFLASH_HIDDEN_NORM, "dflash.hidden_norm" },
            // Per-block surfaces — match Qwen3 layout (GQA + per-head q/k_norm).
            { LLM_TENSOR_ATTN_NORM,          "blk.%d.attn_norm" },
            { LLM_TENSOR_ATTN_Q,             "blk.%d.attn_q" },
            { LLM_TENSOR_ATTN_Q_NORM,        "blk.%d.attn_q_norm" },
            { LLM_TENSOR_ATTN_K,             "blk.%d.attn_k" },
