...
GGML_CALL static bool ggml_backend_cuda_cpy_tensor_async(ggml_backend_t backend_src, ggml_backend_t backend_dst, const ggml_tensor * src, ggml_tensor * dst) {
    ...
    // PHASE_CUDA_NATIVE_DISPATCH C12: GGML_CPY_POST_DEVICE_SYNC env
    // knob deleted. The Phase-46 NPC.4 diagnostic it gated is
    // structurally unnecessary under C1's single-threaded dispatch +
    // C4's outer captured cudaGraph (cross-device ordering is now
    // expressed as captured event-chain edges, not runtime host-side
    // device syncs).
    return true;
}
...
GGML_CALL static void ggml_backend_cuda_synchronize(ggml_backend_t backend) {
    ...
    ggml_cuda_set_device(cuda_ctx->device);
    // PHASE 46 B.5e Phase-C fix: cudaDeviceSynchronize drains ALL
    // streams on the device — needed for libmgpu's multi-device CLIP
    // encoder where peer-access writes land on copy/event streams that
    // a stream-only sync would miss. Perf cost negligible.
    //
    // PHASE_CUDA_NATIVE_DISPATCH C12: env knob escapes
    // GGML_CUDA_STREAM_SYNC and GGML_CUDA_PER_THREAD_SYNC deleted.
...
GGML_CALL static enum ggml_status ggml_backend_cuda_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    if (use_cuda_graph && graph->graph == nullptr) {
        if (ggml_cuda_info().devices[cuda_ctx->device].cc < CC_TURING) {
...
// PHASE_CUDA_NATIVE_DISPATCH C5: end-capture + instantiate (no launch).
// Separated from _end_and_launch so the sched can cache the executable
// across dispatches and replay via _launch_exec on cache hit.
extern "C" bool ggml_cuda_outer_capture_end_capture(
        ggml_backend_t           primary_backend,
        ggml_backend_t         * secondary_backends,
        int                      n_secondary,
        ggml_cuda_graph_exec_t * out_exec) {
    if (out_exec == nullptr) return false;
    *out_exec = nullptr;
    if (primary_backend == nullptr || !ggml_backend_is_cuda(primary_backend)) return false;
    if (!g_outer_capture_active) return false;
    ...
    // Fan-in: every secondary records its completion event; primary
    // waits on each. Same pattern as _end_and_launch.
    for (int i = 0; i < n_secondary; ++i) {
        auto * ctx_i = (ggml_backend_cuda_context *) secondary_backends[i]->context;
        ggml_cuda_set_device(ctx_i->device);
        cudaError_t err = cudaEventRecord(ctx_i->copy_event, ctx_i->stream());
...
extern "C" bool ggml_cuda_outer_capture_launch_exec(
        ggml_backend_t          primary_backend,
...
extern "C" void ggml_cuda_outer_capture_destroy_exec(ggml_cuda_graph_exec_t exec) {
