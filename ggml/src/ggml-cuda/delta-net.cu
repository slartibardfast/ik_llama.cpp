...
static void delta_net_f32_cuda(
    ...
    // Determinism: pin threads_per_block to 256 always. The prior
    // LLAMA_FATTN_SHAPE_INVARIANT_DISPATCH env-gate switched 256↔128 at
    // n_tokens=8, which changes the cross-warp reduction tree (8-way vs
    // 4-way) in reduce_sum() AND the per-step fp32 accumulator order
    // inside the recurrent state update — different bits, accumulated
    // drift across NP. Baked on; gate removed.
    const bool use_256 = true;
    if (use_256) {
