...
#endif
#define GGML_LOG_WARN(...)   printf(__VA_ARGS__)
...
struct vk_device_struct {
    ...
    // Vega (AMD_GCN5) Rapid Packed Math variants — float16_t accumulators map to
    // packed v_pk_fma_f16 instructions and halve VGPR pressure. Created only on
    // AMD_GCN5 with fp16; other devices keep these arrays as nullptr and the
    // selection logic naturally falls back to the f32-accumulator pipelines.
    vk_pipeline pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_COUNT][mul_mat_vec_max_cols];
    vk_pipeline pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_COUNT][mul_mat_vec_max_cols];
    ...
    vk_matmul_pipeline2 pipeline_moe_fused_up_gate[GGML_TYPE_COUNT];
...
struct vk_fused_up_gate_push_constants {
    uint32_t M; uint32_t N; uint32_t K;
    uint32_t stride_a; uint32_t stride_b; uint32_t stride_d;
    uint32_t batch_stride_a; uint32_t batch_stride_b; uint32_t batch_stride_d;
    uint32_t k_split;
    ...
    uint32_t unary_op;
    float limit;
    uint32_t batch_stride_a_gate;
};
struct vk_mat_vec_push_constants {
    uint32_t ncols; uint32_t stride_a; uint32_t stride_b; uint32_t stride_d;
    uint32_t batch_stride_a; uint32_t batch_stride_b; uint32_t batch_stride_d;
    uint32_t fusion_flags; uint32_t base_work_group_y;
    uint32_t ne02; uint32_t ne12; uint32_t broadcast2; uint32_t broadcast3;
    // Runtime column count; every pipeline is compiled at NUM_COLS=max so the
    // spec-const value no longer varies across dispatches. Batch-invariance.
    uint32_t num_cols;
};
...
struct vk_mat_vec_id_push_constants {
    ...
    uint32_t nei0; uint32_t ne11; uint32_t expert_i1; uint32_t nbi1;
    // See vk_mat_vec_push_constants::num_cols.
    uint32_t num_cols;
};
...
#undef CREATE_MM
    ...
    // Subgroup reduction (subgroupAdd) eliminates shared memory barriers.
    // Only safe when BLOCK_SIZE == subgroup_size (single subgroup per workgroup).
    const bool has_sg = device->subgroup_add;
    auto sg_data = [&](auto base_data, auto sg_data, uint32_t bs) -> decltype(base_data) {
        return (has_sg && bs == device->subgroup_size) ? sg_data : base_data;
    };
    auto sg_len = [&](uint64_t base_len, uint64_t sg_len_v, uint32_t bs) -> uint64_t {
        return (has_sg && bs == device->subgroup_size) ? sg_len_v : base_len;
    };

    for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_F32 ][i], "mul_mat_vec_f32_f32_f32_"+std::to_string(i+1),  mul_mat_vec_f32_f32_f32_len,  mul_mat_vec_f32_f32_f32_data,  "main", 5, sizeof(vk_mat_vec_push_constants), {2, 1, 1}, {device->subgroup_size, 2, mul_mat_vec_max_cols}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_F16 ][i], "mul_mat_vec_f16_f32_f32_"+std::to_string(i+1),  mul_mat_vec_f16_f32_f32_len,  mul_mat_vec_f16_f32_f32_data,  "main", 5, sizeof(vk_mat_vec_push_constants), {2, 1, 1}, {device->subgroup_size, 2, mul_mat_vec_max_cols}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_BF16][i], "mul_mat_vec_bf16_f32_f32_"+std::to_string(i+1), mul_mat_vec_bf16_f32_f32_len, mul_mat_vec_bf16_f32_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {2, 1, 1}, {device->subgroup_size, 2, mul_mat_vec_max_cols}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q4_0][i], "mul_mat_vec_q4_0_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_0_f32_f32_len, mul_mat_vec_q4_0_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q4_0_f32_f32_data, mul_mat_vec_q4_0_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q4_1][i], "mul_mat_vec_q4_1_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_1_f32_f32_len, mul_mat_vec_q4_1_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q4_1_f32_f32_data, mul_mat_vec_q4_1_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q5_0][i], "mul_mat_vec_q5_0_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_0_f32_f32_len, mul_mat_vec_q5_0_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q5_0_f32_f32_data, mul_mat_vec_q5_0_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q5_1][i], "mul_mat_vec_q5_1_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_1_f32_f32_len, mul_mat_vec_q5_1_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q5_1_f32_f32_data, mul_mat_vec_q5_1_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q8_0][i], "mul_mat_vec_q8_0_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q8_0_f32_f32_len, mul_mat_vec_q8_0_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q8_0_f32_f32_data, mul_mat_vec_q8_0_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {1*rm_stdq, 1, 1}, {device->subgroup_size, 1*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q2_K][i], "mul_mat_vec_q2_k_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q2_k_f32_f32_len, mul_mat_vec_q2_k_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q2_k_f32_f32_data, mul_mat_vec_q2_k_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q3_K][i], "mul_mat_vec_q3_k_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q3_k_f32_f32_len, mul_mat_vec_q3_k_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q3_k_f32_f32_data, mul_mat_vec_q3_k_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q4_K][i], "mul_mat_vec_q4_k_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_k_f32_f32_len, mul_mat_vec_q4_k_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q4_k_f32_f32_data, mul_mat_vec_q4_k_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q5_K][i], "mul_mat_vec_q5_k_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_k_f32_f32_len, mul_mat_vec_q5_k_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q5_k_f32_f32_data, mul_mat_vec_q5_k_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q6_K][i], "mul_mat_vec_q6_k_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q6_k_f32_f32_len, mul_mat_vec_q6_k_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q6_k_f32_f32_data, mul_mat_vec_q6_k_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ1_S][i],   "mul_mat_vec_iq1_s_f32_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq1_s_f32_f32_len, mul_mat_vec_iq1_s_f32_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq1_s_f32_f32_data, mul_mat_vec_iq1_s_f32_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ1_M][i],   "mul_mat_vec_iq1_m_f32_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq1_m_f32_f32_len, mul_mat_vec_iq1_m_f32_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq1_m_f32_f32_data, mul_mat_vec_iq1_m_f32_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ2_XXS][i], "mul_mat_vec_iq2_xxs_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_iq2_xxs_f32_f32_len, mul_mat_vec_iq2_xxs_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_iq2_xxs_f32_f32_data, mul_mat_vec_iq2_xxs_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ2_XS][i],  "mul_mat_vec_iq2_xs_f32_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq2_xs_f32_f32_len, mul_mat_vec_iq2_xs_f32_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq2_xs_f32_f32_data, mul_mat_vec_iq2_xs_f32_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ2_S][i],   "mul_mat_vec_iq2_s_f32_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq2_s_f32_f32_len, mul_mat_vec_iq2_s_f32_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq2_s_f32_f32_data, mul_mat_vec_iq2_s_f32_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ3_XXS][i], "mul_mat_vec_iq3_xxs_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_iq3_xxs_f32_f32_len, mul_mat_vec_iq3_xxs_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_iq3_xxs_f32_f32_data, mul_mat_vec_iq3_xxs_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ3_S][i],   "mul_mat_vec_iq3_s_f32_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq3_s_f32_f32_len, mul_mat_vec_iq3_s_f32_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq3_s_f32_f32_data, mul_mat_vec_iq3_s_f32_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ4_XS][i],  "mul_mat_vec_iq4_xs_f32_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq4_xs_f32_f32_len, mul_mat_vec_iq4_xs_f32_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq4_xs_f32_f32_data, mul_mat_vec_iq4_xs_f32_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_IQ4_NL][i],  "mul_mat_vec_iq4_nl_f32_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq4_nl_f32_f32_len, mul_mat_vec_iq4_nl_f32_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq4_nl_f32_f32_data, mul_mat_vec_iq4_nl_f32_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);

        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_F32 ][i], "mul_mat_vec_f32_f16_f32_"+std::to_string(i+1),  mul_mat_vec_f32_f16_f32_len,  mul_mat_vec_f32_f16_f32_data,  "main", 5, sizeof(vk_mat_vec_push_constants), {2, 1, 1}, {device->subgroup_size, 2, mul_mat_vec_max_cols}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_F16 ][i], "mul_mat_vec_f16_f16_f32_"+std::to_string(i+1),  mul_mat_vec_f16_f16_f32_len,  mul_mat_vec_f16_f16_f32_data,  "main", 5, sizeof(vk_mat_vec_push_constants), {2, 1, 1}, {device->subgroup_size, 2, mul_mat_vec_max_cols}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_BF16][i], "mul_mat_vec_bf16_f16_f32_"+std::to_string(i+1), mul_mat_vec_bf16_f16_f32_len, mul_mat_vec_bf16_f16_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {2, 1, 1}, {device->subgroup_size, 2, mul_mat_vec_max_cols}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q4_0][i], "mul_mat_vec_q4_0_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_0_f16_f32_len, mul_mat_vec_q4_0_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q4_0_f16_f32_data, mul_mat_vec_q4_0_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q4_1][i], "mul_mat_vec_q4_1_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_1_f16_f32_len, mul_mat_vec_q4_1_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q4_1_f16_f32_data, mul_mat_vec_q4_1_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q5_0][i], "mul_mat_vec_q5_0_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_0_f16_f32_len, mul_mat_vec_q5_0_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q5_0_f16_f32_data, mul_mat_vec_q5_0_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q5_1][i], "mul_mat_vec_q5_1_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_1_f16_f32_len, mul_mat_vec_q5_1_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q5_1_f16_f32_data, mul_mat_vec_q5_1_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q8_0][i], "mul_mat_vec_q8_0_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q8_0_f16_f32_len, mul_mat_vec_q8_0_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q8_0_f16_f32_data, mul_mat_vec_q8_0_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {1*rm_stdq, 1, 1}, {device->subgroup_size, 1*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q2_K][i], "mul_mat_vec_q2_k_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q2_k_f16_f32_len, mul_mat_vec_q2_k_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q2_k_f16_f32_data, mul_mat_vec_q2_k_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q3_K][i], "mul_mat_vec_q3_k_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q3_k_f16_f32_len, mul_mat_vec_q3_k_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q3_k_f16_f32_data, mul_mat_vec_q3_k_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q4_K][i], "mul_mat_vec_q4_k_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_k_f16_f32_len, mul_mat_vec_q4_k_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q4_k_f16_f32_data, mul_mat_vec_q4_k_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q5_K][i], "mul_mat_vec_q5_k_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_k_f16_f32_len, mul_mat_vec_q5_k_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q5_k_f16_f32_data, mul_mat_vec_q5_k_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q6_K][i], "mul_mat_vec_q6_k_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q6_k_f16_f32_len, mul_mat_vec_q6_k_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q6_k_f16_f32_data, mul_mat_vec_q6_k_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ1_S][i],   "mul_mat_vec_iq1_s_f16_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq1_s_f16_f32_len, mul_mat_vec_iq1_s_f16_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq1_s_f16_f32_data, mul_mat_vec_iq1_s_f16_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ1_M][i],   "mul_mat_vec_iq1_m_f16_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq1_m_f16_f32_len, mul_mat_vec_iq1_m_f16_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq1_m_f16_f32_data, mul_mat_vec_iq1_m_f16_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ2_XXS][i], "mul_mat_vec_iq2_xxs_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_iq2_xxs_f16_f32_len, mul_mat_vec_iq2_xxs_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_iq2_xxs_f16_f32_data, mul_mat_vec_iq2_xxs_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ2_XS][i],  "mul_mat_vec_iq2_xs_f16_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq2_xs_f16_f32_len, mul_mat_vec_iq2_xs_f16_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq2_xs_f16_f32_data, mul_mat_vec_iq2_xs_f16_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ2_S][i],   "mul_mat_vec_iq2_s_f16_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq2_s_f16_f32_len, mul_mat_vec_iq2_s_f16_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq2_s_f16_f32_data, mul_mat_vec_iq2_s_f16_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ3_XXS][i], "mul_mat_vec_iq3_xxs_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_iq3_xxs_f16_f32_len, mul_mat_vec_iq3_xxs_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_iq3_xxs_f16_f32_data, mul_mat_vec_iq3_xxs_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ3_S][i],   "mul_mat_vec_iq3_s_f16_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq3_s_f16_f32_len, mul_mat_vec_iq3_s_f16_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq3_s_f16_f32_data, mul_mat_vec_iq3_s_f16_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ4_XS][i],  "mul_mat_vec_iq4_xs_f16_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq4_xs_f16_f32_len, mul_mat_vec_iq4_xs_f16_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq4_xs_f16_f32_data, mul_mat_vec_iq4_xs_f16_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_IQ4_NL][i],  "mul_mat_vec_iq4_nl_f16_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq4_nl_f16_f32_len, mul_mat_vec_iq4_nl_f16_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq4_nl_f16_f32_data, mul_mat_vec_iq4_nl_f16_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);

        // ===== Vega RPM (AMD_GCN5) f16-accumulator variants =====
        // float16_t accumulator → packed v_pk_fma_f16 + halved VGPR pressure → higher
        // occupancy on Vega's wave64 SIMDs. Skip the F32/F16/BF16 input types because
        // their existing path is already optimal and they're not bandwidth-bound the
        // same way quant dequant is. Selection at dispatch time falls back to the
        // f32-acc pipelines on non-Vega devices (these arrays stay nullptr).
        if (device->architecture == AMD_GCN5 && device->fp16) {
            // f32 B input
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q4_0][i], "mul_mat_vec_q4_0_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_0_f16acc_f32_f32_len, mul_mat_vec_q4_0_f16acc_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q4_0_f16acc_f32_f32_data, mul_mat_vec_q4_0_f16acc_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q4_1][i], "mul_mat_vec_q4_1_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_1_f16acc_f32_f32_len, mul_mat_vec_q4_1_f16acc_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q4_1_f16acc_f32_f32_data, mul_mat_vec_q4_1_f16acc_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q5_0][i], "mul_mat_vec_q5_0_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_0_f16acc_f32_f32_len, mul_mat_vec_q5_0_f16acc_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q5_0_f16acc_f32_f32_data, mul_mat_vec_q5_0_f16acc_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q5_1][i], "mul_mat_vec_q5_1_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_1_f16acc_f32_f32_len, mul_mat_vec_q5_1_f16acc_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q5_1_f16acc_f32_f32_data, mul_mat_vec_q5_1_f16acc_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q8_0][i], "mul_mat_vec_q8_0_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q8_0_f16acc_f32_f32_len, mul_mat_vec_q8_0_f16acc_f32_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q8_0_f16acc_f32_f32_data, mul_mat_vec_q8_0_f16acc_f32_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {1*rm_stdq, 1, 1}, {device->subgroup_size, 1*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q2_K][i], "mul_mat_vec_q2_k_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q2_k_f16acc_f32_f32_len, mul_mat_vec_q2_k_f16acc_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q2_k_f16acc_f32_f32_data, mul_mat_vec_q2_k_f16acc_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q3_K][i], "mul_mat_vec_q3_k_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q3_k_f16acc_f32_f32_len, mul_mat_vec_q3_k_f16acc_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q3_k_f16acc_f32_f32_data, mul_mat_vec_q3_k_f16acc_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q4_K][i], "mul_mat_vec_q4_k_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_k_f16acc_f32_f32_len, mul_mat_vec_q4_k_f16acc_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q4_k_f16acc_f32_f32_data, mul_mat_vec_q4_k_f16acc_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q5_K][i], "mul_mat_vec_q5_k_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_k_f16acc_f32_f32_len, mul_mat_vec_q5_k_f16acc_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q5_k_f16acc_f32_f32_data, mul_mat_vec_q5_k_f16acc_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_Q6_K][i], "mul_mat_vec_q6_k_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q6_k_f16acc_f32_f32_len, mul_mat_vec_q6_k_f16acc_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q6_k_f16acc_f32_f32_data, mul_mat_vec_q6_k_f16acc_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ1_S][i],   "mul_mat_vec_iq1_s_f16acc_f32_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq1_s_f16acc_f32_f32_len, mul_mat_vec_iq1_s_f16acc_f32_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq1_s_f16acc_f32_f32_data, mul_mat_vec_iq1_s_f16acc_f32_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ1_M][i],   "mul_mat_vec_iq1_m_f16acc_f32_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq1_m_f16acc_f32_f32_len, mul_mat_vec_iq1_m_f16acc_f32_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq1_m_f16acc_f32_f32_data, mul_mat_vec_iq1_m_f16acc_f32_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ2_XXS][i], "mul_mat_vec_iq2_xxs_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_iq2_xxs_f16acc_f32_f32_len, mul_mat_vec_iq2_xxs_f16acc_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_iq2_xxs_f16acc_f32_f32_data, mul_mat_vec_iq2_xxs_f16acc_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ2_XS][i],  "mul_mat_vec_iq2_xs_f16acc_f32_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq2_xs_f16acc_f32_f32_len, mul_mat_vec_iq2_xs_f16acc_f32_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq2_xs_f16acc_f32_f32_data, mul_mat_vec_iq2_xs_f16acc_f32_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ2_S][i],   "mul_mat_vec_iq2_s_f16acc_f32_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq2_s_f16acc_f32_f32_len, mul_mat_vec_iq2_s_f16acc_f32_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq2_s_f16acc_f32_f32_data, mul_mat_vec_iq2_s_f16acc_f32_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ3_XXS][i], "mul_mat_vec_iq3_xxs_f16acc_f32_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_iq3_xxs_f16acc_f32_f32_len, mul_mat_vec_iq3_xxs_f16acc_f32_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_iq3_xxs_f16acc_f32_f32_data, mul_mat_vec_iq3_xxs_f16acc_f32_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ3_S][i],   "mul_mat_vec_iq3_s_f16acc_f32_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq3_s_f16acc_f32_f32_len, mul_mat_vec_iq3_s_f16acc_f32_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq3_s_f16acc_f32_f32_data, mul_mat_vec_iq3_s_f16acc_f32_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ4_XS][i],  "mul_mat_vec_iq4_xs_f16acc_f32_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq4_xs_f16acc_f32_f32_len, mul_mat_vec_iq4_xs_f16acc_f32_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq4_xs_f16acc_f32_f32_data, mul_mat_vec_iq4_xs_f16acc_f32_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[GGML_TYPE_IQ4_NL][i],  "mul_mat_vec_iq4_nl_f16acc_f32_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq4_nl_f16acc_f32_f32_len, mul_mat_vec_iq4_nl_f16acc_f32_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq4_nl_f16acc_f32_f32_data, mul_mat_vec_iq4_nl_f16acc_f32_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);

            // f16 B input (saves activation-vector bandwidth on top of f16 acc)
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q4_0][i], "mul_mat_vec_q4_0_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_0_f16acc_f16_f32_len, mul_mat_vec_q4_0_f16acc_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q4_0_f16acc_f16_f32_data, mul_mat_vec_q4_0_f16acc_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q4_1][i], "mul_mat_vec_q4_1_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_1_f16acc_f16_f32_len, mul_mat_vec_q4_1_f16acc_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q4_1_f16acc_f16_f32_data, mul_mat_vec_q4_1_f16acc_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q5_0][i], "mul_mat_vec_q5_0_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_0_f16acc_f16_f32_len, mul_mat_vec_q5_0_f16acc_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q5_0_f16acc_f16_f32_data, mul_mat_vec_q5_0_f16acc_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q5_1][i], "mul_mat_vec_q5_1_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_1_f16acc_f16_f32_len, mul_mat_vec_q5_1_f16acc_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q5_1_f16acc_f16_f32_data, mul_mat_vec_q5_1_f16acc_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q8_0][i], "mul_mat_vec_q8_0_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q8_0_f16acc_f16_f32_len, mul_mat_vec_q8_0_f16acc_f16_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_q8_0_f16acc_f16_f32_data, mul_mat_vec_q8_0_f16acc_f16_f32_subgroup_data, device->subgroup_size), "main", 5, sizeof(vk_mat_vec_push_constants), {1*rm_stdq, 1, 1}, {device->subgroup_size, 1*rm_stdq, mul_mat_vec_max_cols}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q2_K][i], "mul_mat_vec_q2_k_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q2_k_f16acc_f16_f32_len, mul_mat_vec_q2_k_f16acc_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q2_k_f16acc_f16_f32_data, mul_mat_vec_q2_k_f16acc_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q3_K][i], "mul_mat_vec_q3_k_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q3_k_f16acc_f16_f32_len, mul_mat_vec_q3_k_f16acc_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q3_k_f16acc_f16_f32_data, mul_mat_vec_q3_k_f16acc_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q4_K][i], "mul_mat_vec_q4_k_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q4_k_f16acc_f16_f32_len, mul_mat_vec_q4_k_f16acc_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q4_k_f16acc_f16_f32_data, mul_mat_vec_q4_k_f16acc_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q5_K][i], "mul_mat_vec_q5_k_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q5_k_f16acc_f16_f32_len, mul_mat_vec_q5_k_f16acc_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q5_k_f16acc_f16_f32_data, mul_mat_vec_q5_k_f16acc_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_Q6_K][i], "mul_mat_vec_q6_k_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_q6_k_f16acc_f16_f32_len, mul_mat_vec_q6_k_f16acc_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_q6_k_f16acc_f16_f32_data, mul_mat_vec_q6_k_f16acc_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ1_S][i],   "mul_mat_vec_iq1_s_f16acc_f16_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq1_s_f16acc_f16_f32_len, mul_mat_vec_iq1_s_f16acc_f16_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq1_s_f16acc_f16_f32_data, mul_mat_vec_iq1_s_f16acc_f16_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ1_M][i],   "mul_mat_vec_iq1_m_f16acc_f16_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq1_m_f16acc_f16_f32_len, mul_mat_vec_iq1_m_f16acc_f16_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq1_m_f16acc_f16_f32_data, mul_mat_vec_iq1_m_f16acc_f16_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ2_XXS][i], "mul_mat_vec_iq2_xxs_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_iq2_xxs_f16acc_f16_f32_len, mul_mat_vec_iq2_xxs_f16acc_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_iq2_xxs_f16acc_f16_f32_data, mul_mat_vec_iq2_xxs_f16acc_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ2_XS][i],  "mul_mat_vec_iq2_xs_f16acc_f16_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq2_xs_f16acc_f16_f32_len, mul_mat_vec_iq2_xs_f16acc_f16_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq2_xs_f16acc_f16_f32_data, mul_mat_vec_iq2_xs_f16acc_f16_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ2_S][i],   "mul_mat_vec_iq2_s_f16acc_f16_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq2_s_f16acc_f16_f32_len, mul_mat_vec_iq2_s_f16acc_f16_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq2_s_f16acc_f16_f32_data, mul_mat_vec_iq2_s_f16acc_f16_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ3_XXS][i], "mul_mat_vec_iq3_xxs_f16acc_f16_f32_"+std::to_string(i+1), sg_len(mul_mat_vec_iq3_xxs_f16acc_f16_f32_len, mul_mat_vec_iq3_xxs_f16acc_f16_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_iq3_xxs_f16acc_f16_f32_data, mul_mat_vec_iq3_xxs_f16acc_f16_f32_subgroup_data, subgroup_size_16), "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ3_S][i],   "mul_mat_vec_iq3_s_f16acc_f16_f32_"+std::to_string(i+1),   sg_len(mul_mat_vec_iq3_s_f16acc_f16_f32_len, mul_mat_vec_iq3_s_f16acc_f16_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_iq3_s_f16acc_f16_f32_data, mul_mat_vec_iq3_s_f16acc_f16_f32_subgroup_data, subgroup_size_16),   "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ4_XS][i],  "mul_mat_vec_iq4_xs_f16acc_f16_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq4_xs_f16acc_f16_f32_len, mul_mat_vec_iq4_xs_f16acc_f16_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq4_xs_f16acc_f16_f32_data, mul_mat_vec_iq4_xs_f16acc_f16_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[GGML_TYPE_IQ4_NL][i],  "mul_mat_vec_iq4_nl_f16acc_f16_f32_"+std::to_string(i+1),  sg_len(mul_mat_vec_iq4_nl_f16acc_f16_f32_len, mul_mat_vec_iq4_nl_f16acc_f16_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_iq4_nl_f16acc_f16_f32_data, mul_mat_vec_iq4_nl_f16acc_f16_f32_subgroup_data, subgroup_size_16),  "main", 5, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq, mul_mat_vec_max_cols}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
        }
    }

    // MMVQ pipelines (quantized A × Q8_1 B, using integer dot product)
    if (device->integer_dot_product) {
        for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q4_0][i], "mul_mat_vec_q4_0_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q4_0_q8_1_f32_len, mul_mat_vec_q4_0_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {device->subgroup_size, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q4_1][i], "mul_mat_vec_q4_1_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q4_1_q8_1_f32_len, mul_mat_vec_q4_1_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {device->subgroup_size, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q5_0][i], "mul_mat_vec_q5_0_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q5_0_q8_1_f32_len, mul_mat_vec_q5_0_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {device->subgroup_size, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q5_1][i], "mul_mat_vec_q5_1_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q5_1_q8_1_f32_len, mul_mat_vec_q5_1_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {device->subgroup_size, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q8_0][i], "mul_mat_vec_q8_0_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q8_0_q8_1_f32_len, mul_mat_vec_q8_0_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {device->subgroup_size, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q2_K][i], "mul_mat_vec_q2_k_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q2_k_q8_1_f32_len, mul_mat_vec_q2_k_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {subgroup_size_16, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q3_K][i], "mul_mat_vec_q3_k_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q3_k_q8_1_f32_len, mul_mat_vec_q3_k_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {subgroup_size_16, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q4_K][i], "mul_mat_vec_q4_k_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q4_k_q8_1_f32_len, mul_mat_vec_q4_k_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {subgroup_size_16, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[GGML_TYPE_Q5_K][i], "mul_mat_vec_q5_k_q8_1_f32_"+std::to_string(i+1), mul_mat_vec_q5_k_q8_1_f32_len, mul_mat_vec_q5_k_q8_1_f32_data, "main", 5, sizeof(vk_mat_vec_push_constants), {1, 1, 1}, {subgroup_size_16, 1, mul_mat_vec_max_cols}, 1, true, false, 0);
        }
    }

    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_F32 ], "mul_mat_vec_id_f32_f32",  mul_mat_vec_id_f32_f32_len,  mul_mat_vec_id_f32_f32_data,  "main", 6, sizeof(vk_mat_vec_id_push_constants), {2, 1, 1}, {device->subgroup_size, 2}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_F16 ], "mul_mat_vec_id_f16_f32",  mul_mat_vec_id_f16_f32_len,  mul_mat_vec_id_f16_f32_data,  "main", 6, sizeof(vk_mat_vec_id_push_constants), {2, 1, 1}, {device->subgroup_size, 2}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_BF16], "mul_mat_vec_id_bf16_f32", mul_mat_vec_id_bf16_f32_len, mul_mat_vec_id_bf16_f32_data, "main", 6, sizeof(vk_mat_vec_id_push_constants), {2, 1, 1}, {device->subgroup_size, 2}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q4_0], "mul_mat_vec_id_q4_0_f32", sg_len(mul_mat_vec_id_q4_0_f32_len, mul_mat_vec_id_q4_0_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_id_q4_0_f32_data, mul_mat_vec_id_q4_0_f32_subgroup_data, device->subgroup_size), "main", 6, sizeof(vk_mat_vec_id_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q4_1], "mul_mat_vec_id_q4_1_f32", sg_len(mul_mat_vec_id_q4_1_f32_len, mul_mat_vec_id_q4_1_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_id_q4_1_f32_data, mul_mat_vec_id_q4_1_f32_subgroup_data, device->subgroup_size), "main", 6, sizeof(vk_mat_vec_id_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q5_0], "mul_mat_vec_id_q5_0_f32", sg_len(mul_mat_vec_id_q5_0_f32_len, mul_mat_vec_id_q5_0_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_id_q5_0_f32_data, mul_mat_vec_id_q5_0_f32_subgroup_data, device->subgroup_size), "main", 6, sizeof(vk_mat_vec_id_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q5_1], "mul_mat_vec_id_q5_1_f32", sg_len(mul_mat_vec_id_q5_1_f32_len, mul_mat_vec_id_q5_1_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_id_q5_1_f32_data, mul_mat_vec_id_q5_1_f32_subgroup_data, device->subgroup_size), "main", 6, sizeof(vk_mat_vec_id_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q8_0], "mul_mat_vec_id_q8_0_f32", sg_len(mul_mat_vec_id_q8_0_f32_len, mul_mat_vec_id_q8_0_f32_subgroup_len, device->subgroup_size), sg_data(mul_mat_vec_id_q8_0_f32_data, mul_mat_vec_id_q8_0_f32_subgroup_data, device->subgroup_size), "main", 6, sizeof(vk_mat_vec_id_push_constants), {1*rm_stdq, 1, 1}, {device->subgroup_size, 1*rm_stdq}, 1, true, has_sg && device->subgroup_size == device->subgroup_size, has_sg && device->subgroup_size == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q2_K], "mul_mat_vec_id_q2_k_f32", sg_len(mul_mat_vec_id_q2_k_f32_len, mul_mat_vec_id_q2_k_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_id_q2_k_f32_data, mul_mat_vec_id_q2_k_f32_subgroup_data, subgroup_size_16), "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q3_K], "mul_mat_vec_id_q3_k_f32", sg_len(mul_mat_vec_id_q3_k_f32_len, mul_mat_vec_id_q3_k_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_id_q3_k_f32_data, mul_mat_vec_id_q3_k_f32_subgroup_data, subgroup_size_16), "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q4_K], "mul_mat_vec_id_q4_k_f32", sg_len(mul_mat_vec_id_q4_k_f32_len, mul_mat_vec_id_q4_k_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_id_q4_k_f32_data, mul_mat_vec_id_q4_k_f32_subgroup_data, subgroup_size_16), "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q5_K], "mul_mat_vec_id_q5_k_f32", sg_len(mul_mat_vec_id_q5_k_f32_len, mul_mat_vec_id_q5_k_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_id_q5_k_f32_data, mul_mat_vec_id_q5_k_f32_subgroup_data, subgroup_size_16), "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q6_K], "mul_mat_vec_id_q6_k_f32", sg_len(mul_mat_vec_id_q6_k_f32_len, mul_mat_vec_id_q6_k_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_id_q6_k_f32_data, mul_mat_vec_id_q6_k_f32_subgroup_data, subgroup_size_16), "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_kq, 1, 1}, {subgroup_size_16, rm_kq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ1_S],   "mul_mat_vec_id_iq1_s_f32",   sg_len(mul_mat_vec_id_iq1_s_f32_len, mul_mat_vec_id_iq1_s_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_id_iq1_s_f32_data, mul_mat_vec_id_iq1_s_f32_subgroup_data, subgroup_size_16),   "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ1_M],   "mul_mat_vec_id_iq1_m_f32",   sg_len(mul_mat_vec_id_iq1_m_f32_len, mul_mat_vec_id_iq1_m_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_id_iq1_m_f32_data, mul_mat_vec_id_iq1_m_f32_subgroup_data, subgroup_size_16),   "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ2_XXS], "mul_mat_vec_id_iq2_xxs_f32", sg_len(mul_mat_vec_id_iq2_xxs_f32_len, mul_mat_vec_id_iq2_xxs_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_id_iq2_xxs_f32_data, mul_mat_vec_id_iq2_xxs_f32_subgroup_data, subgroup_size_16), "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ2_XS],  "mul_mat_vec_id_iq2_xs_f32",  sg_len(mul_mat_vec_id_iq2_xs_f32_len, mul_mat_vec_id_iq2_xs_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_id_iq2_xs_f32_data, mul_mat_vec_id_iq2_xs_f32_subgroup_data, subgroup_size_16),  "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ2_S],   "mul_mat_vec_id_iq2_s_f32",   sg_len(mul_mat_vec_id_iq2_s_f32_len, mul_mat_vec_id_iq2_s_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_id_iq2_s_f32_data, mul_mat_vec_id_iq2_s_f32_subgroup_data, subgroup_size_16),   "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ3_XXS], "mul_mat_vec_id_iq3_xxs_f32", sg_len(mul_mat_vec_id_iq3_xxs_f32_len, mul_mat_vec_id_iq3_xxs_f32_subgroup_len, subgroup_size_16), sg_data(mul_mat_vec_id_iq3_xxs_f32_data, mul_mat_vec_id_iq3_xxs_f32_subgroup_data, subgroup_size_16), "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ3_S],   "mul_mat_vec_id_iq3_s_f32",   sg_len(mul_mat_vec_id_iq3_s_f32_len, mul_mat_vec_id_iq3_s_f32_subgroup_len, subgroup_size_16),   sg_data(mul_mat_vec_id_iq3_s_f32_data, mul_mat_vec_id_iq3_s_f32_subgroup_data, subgroup_size_16),   "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ4_XS],  "mul_mat_vec_id_iq4_xs_f32",  sg_len(mul_mat_vec_id_iq4_xs_f32_len, mul_mat_vec_id_iq4_xs_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_id_iq4_xs_f32_data, mul_mat_vec_id_iq4_xs_f32_subgroup_data, subgroup_size_16),  "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_IQ4_NL],  "mul_mat_vec_id_iq4_nl_f32",  sg_len(mul_mat_vec_id_iq4_nl_f32_len, mul_mat_vec_id_iq4_nl_f32_subgroup_len, subgroup_size_16),  sg_data(mul_mat_vec_id_iq4_nl_f32_data, mul_mat_vec_id_iq4_nl_f32_subgroup_data, subgroup_size_16),  "main", 6, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {subgroup_size_16, rm_iq}, 1, true, has_sg && subgroup_size_16 == device->subgroup_size, has_sg && subgroup_size_16 == device->subgroup_size ? device->subgroup_size : 0);
    ...
    ggml_vk_create_pipeline(device, device->pipeline_get_rows[GGML_TYPE_Q4_K],   "get_rows_q4_k",    get_rows_q4_k_len,    get_rows_q4_k_data,    "main", 3, sizeof(vk_op_binary_push_constants), { 256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_get_rows[GGML_TYPE_Q5_K],   "get_rows_q5_k",    get_rows_q5_k_len,    get_rows_q5_k_data,    "main", 3, sizeof(vk_op_binary_push_constants), { 256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_get_rows[GGML_TYPE_Q6_K],   "get_rows_q6_k",    get_rows_q6_k_len,    get_rows_q6_k_data,    "main", 3, sizeof(vk_op_binary_push_constants), { 256, 1, 1}, {}, 1);
    ...
    if (device->float_controls_rte_fp16) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_Q4_K], "cpy_f32_q4_k", cpy_f32_q4_k_rte_len, cpy_f32_q4_k_rte_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_Q5_K], "cpy_f32_q5_k", cpy_f32_q5_k_rte_len, cpy_f32_q5_k_rte_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_Q6_K], "cpy_f32_q6_k", cpy_f32_q6_k_rte_len, cpy_f32_q6_k_rte_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
    } else {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_Q5_K], "cpy_f32_q5_k", cpy_f32_q5_k_len, cpy_f32_q5_k_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_Q6_K], "cpy_f32_q6_k", cpy_f32_q6_k_len, cpy_f32_q6_k_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
    }
    ...
    if (device->float_controls_rte_fp16) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[GGML_TYPE_Q4_K], "set_rows_q4_k", set_rows_q4_k_len, set_rows_q4_k_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[GGML_TYPE_Q5_K], "set_rows_q5_k", set_rows_q5_k_len, set_rows_q5_k_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[GGML_TYPE_Q6_K], "set_rows_q6_k", set_rows_q6_k_len, set_rows_q6_k_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
    }
    ...
    ggml_vk_create_pipeline(device, device->pipeline_cpy_quant_f32[GGML_TYPE_Q4_K], "cpy_q4_k_f32", cpy_q4_k_f32_len, cpy_q4_k_f32_data, "main", 2, sizeof(vk_op_unary_push_constants), {1, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_cpy_quant_f32[GGML_TYPE_Q5_K], "cpy_q5_k_f32", cpy_q5_k_f32_len, cpy_q5_k_f32_data, "main", 2, sizeof(vk_op_unary_push_constants), {1, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_cpy_quant_f32[GGML_TYPE_Q6_K], "cpy_q6_k_f32", cpy_q6_k_f32_len, cpy_q6_k_f32_data, "main", 2, sizeof(vk_op_unary_push_constants), {1, 1, 1}, {}, 1);
...
#undef CREATE_GLU
    ...
    // Fused up-gate dual-matmul pipelines (one per quant type, 4 bindings: up, b, dst, gate)
#define CREATE_FUG(TYPE, NAMELC) \
    if (device->mul_mat_l[TYPE]) { \
        ggml_vk_create_pipeline(device, device->pipeline_fused_up_gate[TYPE].f32acc->l, #NAMELC "_l", NAMELC ## _len, NAMELC ## _data, "main", 4, sizeof(vk_fused_up_gate_push_constants), l_mmq_wg_denoms, l_warptile_mmq, 1); \
        ggml_vk_create_pipeline(device, device->pipeline_fused_up_gate[TYPE].f32acc->a_l, #NAMELC "_aligned_l", NAMELC ## _aligned_len, NAMELC ## _aligned_data, "main", 4, sizeof(vk_fused_up_gate_push_constants), l_mmq_wg_denoms, l_warptile_mmq, l_align); \
    } \
    if (device->mul_mat_m[TYPE]) { \
        ggml_vk_create_pipeline(device, device->pipeline_fused_up_gate[TYPE].f32acc->m, #NAMELC "_m", NAMELC ## _len, NAMELC ## _data, "main", 4, sizeof(vk_fused_up_gate_push_constants), m_mmq_wg_denoms, m_warptile_mmq, 1); \
        ggml_vk_create_pipeline(device, device->pipeline_fused_up_gate[TYPE].f32acc->a_m, #NAMELC "_aligned_m", NAMELC ## _aligned_len, NAMELC ## _aligned_data, "main", 4, sizeof(vk_fused_up_gate_push_constants), m_mmq_wg_denoms, m_warptile_mmq, m_align); \
    } \
    if (device->mul_mat_s[TYPE]) { \
        ggml_vk_create_pipeline(device, device->pipeline_fused_up_gate[TYPE].f32acc->s, #NAMELC "_s", NAMELC ## _len, NAMELC ## _data, "main", 4, sizeof(vk_fused_up_gate_push_constants), s_mmq_wg_denoms, s_warptile_mmq, 1); \
        ggml_vk_create_pipeline(device, device->pipeline_fused_up_gate[TYPE].f32acc->a_s, #NAMELC "_aligned_s", NAMELC ## _aligned_len, NAMELC ## _aligned_data, "main", 4, sizeof(vk_fused_up_gate_push_constants), s_mmq_wg_denoms, s_warptile_mmq, s_align); \
    }

    CREATE_FUG(GGML_TYPE_Q4_0, matmul_fused_up_gate_q4_0_f32)
    CREATE_FUG(GGML_TYPE_Q4_1, matmul_fused_up_gate_q4_1_f32)
    CREATE_FUG(GGML_TYPE_Q5_0, matmul_fused_up_gate_q5_0_f32)
    CREATE_FUG(GGML_TYPE_Q5_1, matmul_fused_up_gate_q5_1_f32)
    CREATE_FUG(GGML_TYPE_Q8_0, matmul_fused_up_gate_q8_0_f32)
    CREATE_FUG(GGML_TYPE_Q2_K, matmul_fused_up_gate_q2_k_f32)
    CREATE_FUG(GGML_TYPE_Q3_K, matmul_fused_up_gate_q3_k_f32)
    CREATE_FUG(GGML_TYPE_Q4_K, matmul_fused_up_gate_q4_k_f32)
    CREATE_FUG(GGML_TYPE_Q5_K, matmul_fused_up_gate_q5_k_f32)
    CREATE_FUG(GGML_TYPE_Q6_K, matmul_fused_up_gate_q6_k_f32)
    // IQ types backported from mul_mm.comp to mul_mm_fused_up_gate.comp.
    CREATE_FUG(GGML_TYPE_IQ1_S,   matmul_fused_up_gate_iq1_s_f32)
    CREATE_FUG(GGML_TYPE_IQ1_M,   matmul_fused_up_gate_iq1_m_f32)
    CREATE_FUG(GGML_TYPE_IQ2_XXS, matmul_fused_up_gate_iq2_xxs_f32)
    CREATE_FUG(GGML_TYPE_IQ2_XS,  matmul_fused_up_gate_iq2_xs_f32)
    CREATE_FUG(GGML_TYPE_IQ2_S,   matmul_fused_up_gate_iq2_s_f32)
    CREATE_FUG(GGML_TYPE_IQ3_XXS, matmul_fused_up_gate_iq3_xxs_f32)
    CREATE_FUG(GGML_TYPE_IQ3_S,   matmul_fused_up_gate_iq3_s_f32)
    CREATE_FUG(GGML_TYPE_IQ4_XS,  matmul_fused_up_gate_iq4_xs_f32)
    CREATE_FUG(GGML_TYPE_IQ4_NL,  matmul_fused_up_gate_iq4_nl_f32)
#undef CREATE_FUG

    // MOE_FUSED_UP_GATE pipelines — same shader as the dense fused_up_gate
    // but compiled with MUL_MAT_ID=1, 5 bindings: up, b, dst, gate, ids.
#define CREATE_FUG_ID(TYPE, NAMELC) \
    if (device->mul_mat_l[TYPE]) { \
        ggml_vk_create_pipeline(device, device->pipeline_moe_fused_up_gate[TYPE].f32acc->l, #NAMELC "_l", NAMELC ## _len, NAMELC ## _data, "main", 5, sizeof(vk_fused_up_gate_push_constants), l_mmq_wg_denoms, l_warptile_mmq, 1); \
        ggml_vk_create_pipeline(device, device->pipeline_moe_fused_up_gate[TYPE].f32acc->a_l, #NAMELC "_aligned_l", NAMELC ## _aligned_len, NAMELC ## _aligned_data, "main", 5, sizeof(vk_fused_up_gate_push_constants), l_mmq_wg_denoms, l_warptile_mmq, l_align); \
    } \
    if (device->mul_mat_m[TYPE]) { \
        ggml_vk_create_pipeline(device, device->pipeline_moe_fused_up_gate[TYPE].f32acc->m, #NAMELC "_m", NAMELC ## _len, NAMELC ## _data, "main", 5, sizeof(vk_fused_up_gate_push_constants), m_mmq_wg_denoms, m_warptile_mmq, 1); \
        ggml_vk_create_pipeline(device, device->pipeline_moe_fused_up_gate[TYPE].f32acc->a_m, #NAMELC "_aligned_m", NAMELC ## _aligned_len, NAMELC ## _aligned_data, "main", 5, sizeof(vk_fused_up_gate_push_constants), m_mmq_wg_denoms, m_warptile_mmq, m_align); \
    } \
    if (device->mul_mat_s[TYPE]) { \
        ggml_vk_create_pipeline(device, device->pipeline_moe_fused_up_gate[TYPE].f32acc->s, #NAMELC "_s", NAMELC ## _len, NAMELC ## _data, "main", 5, sizeof(vk_fused_up_gate_push_constants), s_mmq_wg_denoms, s_warptile_mmq, 1); \
        ggml_vk_create_pipeline(device, device->pipeline_moe_fused_up_gate[TYPE].f32acc->a_s, #NAMELC "_aligned_s", NAMELC ## _aligned_len, NAMELC ## _aligned_data, "main", 5, sizeof(vk_fused_up_gate_push_constants), s_mmq_wg_denoms, s_warptile_mmq, s_align); \
    }

    CREATE_FUG_ID(GGML_TYPE_Q4_0,    matmul_id_fused_up_gate_q4_0_f32)
    CREATE_FUG_ID(GGML_TYPE_Q4_1,    matmul_id_fused_up_gate_q4_1_f32)
    CREATE_FUG_ID(GGML_TYPE_Q5_0,    matmul_id_fused_up_gate_q5_0_f32)
    CREATE_FUG_ID(GGML_TYPE_Q5_1,    matmul_id_fused_up_gate_q5_1_f32)
    CREATE_FUG_ID(GGML_TYPE_Q8_0,    matmul_id_fused_up_gate_q8_0_f32)
    CREATE_FUG_ID(GGML_TYPE_Q2_K,    matmul_id_fused_up_gate_q2_k_f32)
    CREATE_FUG_ID(GGML_TYPE_Q3_K,    matmul_id_fused_up_gate_q3_k_f32)
    CREATE_FUG_ID(GGML_TYPE_Q4_K,    matmul_id_fused_up_gate_q4_k_f32)
    CREATE_FUG_ID(GGML_TYPE_Q5_K,    matmul_id_fused_up_gate_q5_k_f32)
    CREATE_FUG_ID(GGML_TYPE_Q6_K,    matmul_id_fused_up_gate_q6_k_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ1_S,   matmul_id_fused_up_gate_iq1_s_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ1_M,   matmul_id_fused_up_gate_iq1_m_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ2_XXS, matmul_id_fused_up_gate_iq2_xxs_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ2_XS,  matmul_id_fused_up_gate_iq2_xs_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ2_S,   matmul_id_fused_up_gate_iq2_s_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ3_XXS, matmul_id_fused_up_gate_iq3_xxs_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ3_S,   matmul_id_fused_up_gate_iq3_s_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ4_XS,  matmul_id_fused_up_gate_iq4_xs_f32)
    CREATE_FUG_ID(GGML_TYPE_IQ4_NL,  matmul_id_fused_up_gate_iq4_nl_f32)
#undef CREATE_FUG_ID
    //
    // ================================ ik_llama.cpp pipelines end =========================================
...
static vk_pipeline ggml_vk_get_dequantize_mul_mat_vec(ggml_backend_vk_context * ctx, ggml_type a_type, ggml_type b_type, uint32_t num_cols, uint32_t m, uint32_t k) {
    ...
    static const bool stats_enabled = []() {
        const char * v = std::getenv("GGML_VK_F16ACC_STATS");
        if (v && v[0] && v[0] != '0') {
            std::atexit([]() {
                fprintf(stderr, "[f16acc] hits=%llu  f32acc fallbacks=%llu\n",
                    (unsigned long long)f16acc_hits.load(),
                    (unsigned long long)f32acc_hits.load());
            });
            return true;
        }
        return false;
    ...
    if (!disable_f16acc && ctx->device->architecture == AMD_GCN5 && ctx->device->fp16) {
        if (b_type == GGML_TYPE_F32 &&
            ctx->device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[a_type][num_cols-1]) {
            if (stats_enabled) f16acc_hits.fetch_add(1, std::memory_order_relaxed);
            return ctx->device->pipeline_dequant_mul_mat_vec_f16acc_f32_f32[a_type][num_cols-1];
        }
        if (b_type == GGML_TYPE_F16 &&
            ctx->device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[a_type][num_cols-1]) {
            if (stats_enabled) f16acc_hits.fetch_add(1, std::memory_order_relaxed);
            return ctx->device->pipeline_dequant_mul_mat_vec_f16acc_f16_f32[a_type][num_cols-1];
        }
    }
    if (stats_enabled) f32acc_hits.fetch_add(1, std::memory_order_relaxed);
...
static vk_pipeline ggml_vk_get_quantize_pipeline(ggml_backend_vk_context * ctx, ggml_type type, bool x4 = false) {
    switch(type) {
        case GGML_TYPE_Q8_1:
            return x4 ? ctx->device->pipeline_quantize_q8_1_x4 : ctx->device->pipeline_quantize_q8_1;
        default:
...
static void ggml_vk_quantize_q8_1(ggml_backend_vk_context * ctx, vk_context& subctx, const vk_subbuffer & in, const vk_subbuffer & out, uint32_t ne, bool x4 = false) {
    VK_LOG_DEBUG("ggml_vk_quantize_q8_1(" << "buffer in size=" << in.buffer->size << ", buffer out size=" << out.buffer->size << ", " << ne << ", x4=" << x4 << ")");

    vk_pipeline pipeline = ggml_vk_get_quantize_pipeline(ctx, GGML_TYPE_Q8_1, x4);

    const uint32_t num_blocks = CEIL_DIV(ne, pipeline->wg_denoms[0]);
    const uint64_t max_elements = std::min<uint64_t>(
        uint64_t{ctx->device->properties.limits.maxComputeWorkGroupCount[0]} * pipeline->wg_denoms[0],
        std::numeric_limits<uint32_t>::max());
    const uint32_t elements = std::min(ne, static_cast<uint32_t>(max_elements));

    const vk_quantize_q8_1_push_constants pc = { ne, num_blocks };
...
static void ggml_vk_mul_mat_vec_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool dryrun = false) {
    ...
    // Try MMVQ (integer dot product) path first
    bool quantize_y = ctx->device->integer_dot_product
                   && src1->type == GGML_TYPE_F32
                   && ggml_is_contiguous(src1)
                   && !y_non_contig
                   && (ne11 * ne10) % 4 == 0
                   && ggml_vk_should_use_mmvq(ctx, ne01, ne11, ne10, src0->type);
    ...
    // Check for MMVQ first
    vk_pipeline dmmv = quantize_y ? ggml_vk_get_dequantize_mul_mat_vec(ctx, src0->type, GGML_TYPE_Q8_1, ne11, ne20, ne00) : nullptr;
    vk_pipeline to_q8_1 = nullptr;

    if (dmmv == nullptr) {
        // Fall back to dequant mul mat vec
        dmmv = ggml_vk_get_dequantize_mul_mat_vec(ctx, src0->type, src1->type, ne11, ne20, ne00);
        quantize_y = false;
    }

    // Fall back to non-MMVQ if src0 exceeds maxStorageBufferRange (no 64-bit indexing support)
    if (quantize_y && ggml_nbytes(src0) > ctx->device->properties.limits.maxStorageBufferRange) {
        dmmv = ggml_vk_get_dequantize_mul_mat_vec(ctx, src0->type, src1->type, ne11, ne20, ne00);
        quantize_y = false;
    }

    if (quantize_y) {
        // MMVQ path reads block_q8_1_x4, so we must quantize into the x4 format.
    ...
    if (dryrun) {
        if (
                (qx_needs_dequant && x_sz > ctx->device->max_memory_allocation_size) ||
                (qy_needs_dequant && y_sz > ctx->device->max_memory_allocation_size)) {
            GGML_ABORT("Requested preallocation size is too large");
        }
        ...
        ggml_pipeline_request_descriptor_sets(ctx, dmmv,
            CEIL_DIV(ne12 * ne13, ctx->device->properties.limits.maxComputeWorkGroupCount[1]));
        return;
    }
    ...
    if (quantize_y) {
        ggml_vk_quantize_q8_1(ctx, subctx, d_Qy, d_Y, y_ne, true);
    }
...
static void ggml_vk_fused_up_gate(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * up   = dst->src[0];  // quantized weight [K, N, 1, 1]
    const ggml_tensor * gate = dst->src[1];  // quantized weight [K, N, 1, 1]
    const ggml_tensor * b    = dst->src[2];  // input F32 [K, M, 1, 1]

    GGML_ASSERT(ggml_is_quantized(up->type));
    GGML_ASSERT(up->type == gate->type);
    GGML_ASSERT(b->type == GGML_TYPE_F32);

    const uint64_t ne00 = up->ne[0];   // K
    const uint64_t ne01 = up->ne[1];   // N (weight rows = output dim 0)
    const uint64_t ne02 = up->ne[2];
    const uint64_t ne03 = up->ne[3];

    const uint64_t ne20 = b->ne[0];    // K
    const uint64_t ne21 = b->ne[1];    // M (batch = output dim 1)
    const uint64_t ne22 = b->ne[2];
    const uint64_t ne23 = b->ne[3];

    const uint64_t r2 = ne22 / ne02;
    const uint64_t r3 = ne23 / ne03;

    // Get pipeline for this quant type
    vk_matmul_pipeline mmp = ctx->device->pipeline_fused_up_gate[up->type].f32acc;
    GGML_ASSERT(mmp != nullptr);

    const uint32_t kpad = ggml_vk_align_size(ne00, ggml_vk_guess_matmul_pipeline_align(ctx, mmp, ne01, ne21, up->type, GGML_TYPE_F32));
    const bool aligned = ne00 == kpad && ne01 > 8 && ne21 > 8;

    vk_pipeline pipeline = ggml_vk_guess_matmul_pipeline(ctx, mmp, ne01, ne21, aligned, up->type, GGML_TYPE_F32);

    const uint32_t padded_n = (uint32_t)ne21;
    ...
    // Get buffers
    ggml_backend_vk_buffer_context * up_buf_ctx   = (ggml_backend_vk_buffer_context *)up->buffer->context;
    ggml_backend_vk_buffer_context * gate_buf_ctx = (ggml_backend_vk_buffer_context *)gate->buffer->context;
    ggml_backend_vk_buffer_context * b_buf_ctx    = (ggml_backend_vk_buffer_context *)b->buffer->context;
    ggml_backend_vk_buffer_context * dst_buf_ctx  = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    vk_buffer d_up   = up_buf_ctx->dev_buffer;
    vk_buffer d_gate = gate_buf_ctx->dev_buffer;
    vk_buffer d_b    = b_buf_ctx->dev_buffer;
    vk_buffer d_D    = dst_buf_ctx->dev_buffer;

    const uint64_t up_buf_offset   = vk_tensor_offset(up) + up->view_offs;
    const uint64_t gate_buf_offset = vk_tensor_offset(gate) + gate->view_offs;
    const uint64_t b_buf_offset    = vk_tensor_offset(b) + b->view_offs;
    const uint64_t d_buf_offset    = vk_tensor_offset(dst) + dst->view_offs;

    const uint64_t up_sz   = ggml_type_size(up->type) * ne00 * ne01 / ggml_blck_size(up->type);
    const uint64_t gate_sz = ggml_type_size(gate->type) * gate->ne[0] * gate->ne[1] / ggml_blck_size(gate->type);
    const uint64_t b_sz    = sizeof(float) * ne20 * ne21;
    const uint64_t d_sz    = sizeof(float) * ne01 * ne21;

    // Read op params: unary_op and limit
    const int32_t unary_op_ggml = dst->op_params[0];
    const float limit = *(const float *)(dst->op_params + 1);

    uint32_t shader_unary_op;
    switch (unary_op_ggml) {
    case GGML_UNARY_OP_SILU: shader_unary_op = 0; break;
    case GGML_UNARY_OP_GELU: shader_unary_op = 1; break;
    case GGML_UNARY_OP_RELU: shader_unary_op = 2; break;
    default:
        GGML_ABORT("unsupported activation in fused_up_gate");
    }

    const uint32_t batch = (uint32_t)(ne22 * ne23);

    const vk_fused_up_gate_push_constants pc = {
        (uint32_t)ne01,                             // M
        (uint32_t)ne21,                             // N
        (uint32_t)ne00,                             // K
        (uint32_t)ne00,                             // stride_a
        (uint32_t)ne20,                             // stride_b
        (uint32_t)(dst->ne[0]),                     // stride_d
        (uint32_t)(ne00 * ne01),                    // batch_stride_a
        (uint32_t)(ne20 * ne21),                    // batch_stride_b
        (uint32_t)(dst->ne[0] * dst->ne[1]),        // batch_stride_d
        1,                                           // k_split (forced to 1)
        (uint32_t)ne02, (uint32_t)ne22,             // ne02, ne12
        (uint32_t)r2, (uint32_t)r3,                 // broadcast2, broadcast3
        padded_n,
        shader_unary_op,
        limit,
        (uint32_t)(gate->ne[0] * gate->ne[1]),     // batch_stride_a_gate
    };

    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline,
        {
            vk_subbuffer{ d_up,   up_buf_offset,   up_sz * ne02 * ne03 },
            vk_subbuffer{ d_b,    b_buf_offset,    b_sz * ne22 * ne23 },
            vk_subbuffer{ d_D,    d_buf_offset,    d_sz * ne22 * ne23 },
            vk_subbuffer{ d_gate, gate_buf_offset, gate_sz * ne02 * ne03 },
        },
        pc,
        { (uint32_t)ne01, (uint32_t)ne21, batch });
}

// MoE variant of fused_up_gate. Mirrors ggml_vk_fused_up_gate but adds
// expert-ID handling (binding 4) and dispatches one workgroup-grid layer per
// expert in the z dimension. Push constants reuse the dense struct layout via
// macro aliases inside the GLSL shader.
static void ggml_vk_moe_fused_up_gate(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * up   = dst->src[0];  // up   experts [K, N, n_experts]
    const ggml_tensor * gate = dst->src[1];  // gate experts [K, N, n_experts]
    const ggml_tensor * b    = dst->src[2];  // input        [K, n_tokens, ...]
    const ggml_tensor * ids  = dst->src[3];  // expert ids   [n_expert_used, n_tokens]
    ...
    const uint64_t ne00 = up->ne[0];   // K (hidden dim)
    const uint64_t ne01 = up->ne[1];   // N (n_ff per expert = output dim)
    const uint64_t ne02 = up->ne[2];   // n_experts (z dispatch)

    const uint64_t ne10 = b->ne[0];    // K
    const uint64_t ne11 = b->ne[1];    // n_tokens (in input)

    const uint64_t nei0 = ids->ne[0];  // n_expert_used per token
    const uint64_t nei1 = ids->ne[1];  // n_tokens

    GGML_ASSERT(nei0 * nei1 <= 4096);  // row_ids[] capacity in shader

    const uint32_t nbi1 = (uint32_t)(ids->nb[1] / ggml_type_size(ids->type));

    // Pick the f32-acc pipeline. f16acc-on-Vega backport is queued for later.
    vk_matmul_pipeline mmp = ctx->device->pipeline_moe_fused_up_gate[up->type].f32acc;
    GGML_ASSERT(mmp != nullptr);

    const uint32_t kpad = ggml_vk_align_size(ne00, ggml_vk_guess_matmul_pipeline_align(ctx, mmp, ne01, nei1, up->type, GGML_TYPE_F32));
    // For MoE, the inner kernel iterates _ne1 (per-expert columns), not the
    // global N. Aligned variant requires the kernel's M and N tile bounds to
    // match exact tile boundaries — for MoE we use the unaligned form to keep
    // the row_ids indirection simple. (Matches mul_mm_id behavior in mul_mm.comp.)
    const bool aligned = ne00 == kpad && ne01 > 8 && nei1 > 8;
    vk_pipeline pipeline = ggml_vk_guess_matmul_pipeline(ctx, mmp, ne01, nei1, aligned, up->type, GGML_TYPE_F32);
    ...
    vk_buffer d_b    = b_buf_ctx->dev_buffer;
    vk_buffer d_D    = dst_buf_ctx->dev_buffer;
    vk_buffer d_ids  = ids_buf_ctx->dev_buffer;

    const uint64_t up_buf_offset   = vk_tensor_offset(up)   + up->view_offs;
    const uint64_t gate_buf_offset = vk_tensor_offset(gate) + gate->view_offs;
    const uint64_t b_buf_offset    = vk_tensor_offset(b)    + b->view_offs;
    const uint64_t d_buf_offset    = vk_tensor_offset(dst)  + dst->view_offs;
    const uint64_t ids_buf_offset  = vk_tensor_offset(ids)  + ids->view_offs;

    // Per-expert weight footprint, summed across n_experts.
    const uint64_t up_sz   = ggml_type_size(up->type)   * ne00 * ne01 / ggml_blck_size(up->type);
    const uint64_t gate_sz = ggml_type_size(gate->type) * gate->ne[0] * gate->ne[1] / ggml_blck_size(gate->type);
    // b is shape [k, ne11, n_tokens]; the shader steps row_idx.y * batch_stride_b
    // = row_idx.y * (ne10 * ne11) to walk between tokens, so we need the FULL
    // tensor visible to the binding (not just a single token's worth).
    const uint64_t b_sz    = sizeof(float) * ne10 * ne11 * b->ne[2];
    // Output: [n_ff, n_expert_used, n_tokens]
    const uint64_t d_sz    = sizeof(float) * dst->ne[0] * dst->ne[1] * dst->ne[2];
    const uint64_t ids_sz  = sizeof(int32_t) * nei0 * nei1;

    // Read op params: unary_op and limit (same encoding as dense FUSED_UP_GATE)
    const int32_t unary_op_ggml = dst->op_params[0];
    const float limit = *(const float *)(dst->op_params + 1);
    ...
    case GGML_UNARY_OP_RELU: shader_unary_op = 2; break;
    default:
        GGML_ABORT("unsupported activation in moe_fused_up_gate");
    }

    // Push constants — reuse the dense struct layout. The first 5 of the
    // "k_split/ne0X/broadcast*" slots are reinterpreted by the shader's
    // p_nei0/nei1/nbi1/ne11/(unused) macro aliases. The dispatch grid uses
    // gl_GlobalInvocationID.z as expert_idx, so the third grid dim = n_experts.
    const vk_fused_up_gate_push_constants pc = {
        (uint32_t)ne01,                             // M  = n_ff per expert
        (uint32_t)nei1,                             // N  = n_tokens (the inner kernel uses _ne1)
        (uint32_t)ne00,                             // K  = hidden dim
        (uint32_t)ne00,                             // stride_a
        (uint32_t)ne10,                             // stride_b
        (uint32_t)dst->ne[0],                       // stride_d
        (uint32_t)(ne00 * ne01),                    // batch_stride_a (per-expert weight stride)
        (uint32_t)(ne10 * ne11),                    // batch_stride_b
        (uint32_t)(dst->ne[0] * dst->ne[1]),        // batch_stride_d (per-token output stride)
        (uint32_t)nei0,                             // <- p_nei0  (reuses k_split slot)
        (uint32_t)nei1,                             // <- p_nei1  (reuses ne02 slot)
        (uint32_t)nbi1,                             // <- p_nbi1  (reuses ne12 slot)
        (uint32_t)ne11,                             // <- p_ne11  (reuses broadcast2 slot)
        0,                                           // unused MoE pad (broadcast3 slot)
        (uint32_t)nei1,                             // padded_N
        shader_unary_op,
        limit,
        (uint32_t)(gate->ne[0] * gate->ne[1]),     // batch_stride_a_gate
    };

    // Workgroup grid: ggml_vk_dispatch_pipeline auto-divides by wg_denoms
    // (BM, BN, 1), so we pass raw output dims here. Same convention as the
    // existing ggml_vk_matmul_id helper.
    //   x = M           → divided by BM → blocks_m
    //   y = nei1        → divided by BN → blocks_n  (n_tokens; the shader
    //                     scans ids and early-exits if a workgroup column
    //                     has no work for its expert)
    //   z = n_experts   → 1 workgroup-grid per expert
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline,
        {
            vk_subbuffer{ d_up,   up_buf_offset,   up_sz * ne02 },
            vk_subbuffer{ d_b,    b_buf_offset,    b_sz },
            vk_subbuffer{ d_D,    d_buf_offset,    d_sz },
            vk_subbuffer{ d_gate, gate_buf_offset, gate_sz * ne02 },
            vk_subbuffer{ d_ids,  ids_buf_offset,  ids_sz },
        },
        pc,
        { (uint32_t)ne01, (uint32_t)nei1, (uint32_t)ne02 });
}
...
static void ggml_vk_mul_mat_vec_id_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids, ggml_tensor * dst, bool dryrun = false) {
    ...
    const uint64_t nbi1 = ids->nb[1] / sizeof(int);
    ...
    const uint64_t ids_sz = nbi2;
    const uint64_t d_sz = sizeof(float) * ggml_nelements(dst);
    ...
    // compute — batch_stride_a is per-expert (ne00*ne01), not total x_ne
    const vk_mat_vec_id_push_constants pc = {
        (uint32_t)ne00, (uint32_t)ne10, (uint32_t)ne10, (uint32_t)ne01,
        (uint32_t)(ne00 * ne01), stride_batch_y, (uint32_t)(ne20*ne21),
        0,
        (uint32_t)nei0, (uint32_t)ne11, 0, (uint32_t)nbi1,
        1,  // num_cols: MoE mat-vec-id uses NUM_COLS=1 pipelines
    };
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, dmmv,
        { vk_subbuffer{ d_X, x_buf_offset, x_sz },
          vk_subbuffer{ d_Y, y_buf_offset, y_sz }, vk_subbuffer{ d_D, d_buf_offset, d_sz },
...
static void ggml_vk_l2_norm(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, ggml_tensor * dst, bool dryrun = false) {
    ...
    ggml_vk_op_f32<vk_op_unary_push_constants>(ctx, subctx, src0, nullptr, nullptr, dst, GGML_OP_L2_NORM, std::move(p), dryrun);
}
...
static bool ggml_vk_build_graph(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int node_idx, ggml_tensor *node_begin, int node_idx_begin, bool dryrun, bool last_node, bool almost_ready, bool submit){
    ...
    case GGML_OP_FUSED_UP_GATE:
        ggml_vk_fused_up_gate(ctx, compute_ctx, node, dryrun);

        break;

    case GGML_OP_MOE_FUSED_UP_GATE:
        ggml_vk_moe_fused_up_gate(ctx, compute_ctx, node, dryrun);
...
static bool ggml_backend_vk_supports_op(ggml_backend_t backend, const ggml_tensor * op) {
    switch (op->op) {
        ...
        case GGML_OP_FUSED_UP_GATE:
            {
                const ggml_tensor * up = op->src[0];
                const ggml_tensor * gate = op->src[1];
                const ggml_tensor * b = op->src[2];
                if (!up || !gate || !b) return false;
                if (!ggml_is_quantized(up->type)) return false;
                if (up->type != gate->type) return false;
                if (b->type != GGML_TYPE_F32) return false;
                if (!ggml_are_same_shape(up, gate)) return false;
                // Check we have a pipeline for this quant type
                ggml_backend_vk_context * vk_ctx = (ggml_backend_vk_context *)backend->context;
                auto & dev = vk_ctx->device;
                if (!dev->pipeline_fused_up_gate[up->type].f32acc->s &&
                    !dev->pipeline_fused_up_gate[up->type].f32acc->m &&
                    !dev->pipeline_fused_up_gate[up->type].f32acc->l) return false;
                return true;
            }
        case GGML_OP_MOE_FUSED_UP_GATE:
            {
                ...
                if (!up || !gate || !b || !ids) return false;
                if (!ggml_is_quantized(up->type)) return false;
                if (up->type != gate->type) return false;
                if (b->type != GGML_TYPE_F32) return false;
                if (ids->type != GGML_TYPE_I32) return false;
                if (!ggml_are_same_shape(up, gate)) return false;
                // The shader caps row_ids[] at 4096 — refuse larger MoE batches
                // (the caller should split). Matches mul_mat_id_q_f16 limit.
                if ((uint64_t)ids->ne[0] * (uint64_t)ids->ne[1] > 4096) return false;
                ggml_backend_vk_context * vk_ctx = (ggml_backend_vk_context *)backend->context;
                auto & dev = vk_ctx->device;
                if (!dev->pipeline_moe_fused_up_gate[up->type].f32acc->s &&
                    !dev->pipeline_moe_fused_up_gate[up->type].f32acc->m &&
                    !dev->pipeline_moe_fused_up_gate[up->type].f32acc->l) return false;
                return true;
            }
        case GGML_OP_NORM:
        case GGML_OP_GROUP_NORM:
