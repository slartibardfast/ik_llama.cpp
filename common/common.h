...
struct gpt_params {
    ...
    int32_t prefill_chunk_budget  =       0; // per-tick chunked-prefill admission budget K (Sarathi-Serve). 0 = default = n_ubatch.
    ...
    std::string cpu_mask;           // CPU affinity mask for the dispatch thread; hex ("0xF0") or range ("4-7,12-15"). Empty = no change. Linux only.
